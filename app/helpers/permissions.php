        ...
        foreach($crudPermissionsMap as $permission => $methods)
        {
            if(in_array($method, $methods) && isset($classesMap[$controller]))
            {
                ...
                if($className == 'post' && in_array($method, ['edit', 'update', 'destroy', 'restore', 'forceDestroy']))
                {
                    ...
                    //if current user has not update-others-post/delete-others-posts permission
                    //make sure user only modifies theirs own post
                    if($id  &&
                ...
                //if the user has not permissions dont allow the next request
